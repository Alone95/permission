...
public class SysAclModuleService {
    ...
    public void save(AclModuleParam param){
        ...
        if(checkExist(param.getParentId(), param.getName(), param.getId())) {
            throw new ParamException("同一层级下存在相同名称的部门权限模块");
        }
    ...
    private void updateWithChild(SysAclModule before,SysAclModule after){
        ...
        if (!after.getLevel().equals(before.getLevel())) {
            List<SysAclModule> aclModuletList = sysAclModuleMapper.getChildAclModuleListByLevel(before.getLevel());
            if (CollectionUtils.isNotEmpty(aclModuletList)) {
                for (SysAclModule aclModule : aclModuletList) {
                ...
                sysAclModuleMapper.batchUpdateLevel(aclModuletList);
            }
        }
